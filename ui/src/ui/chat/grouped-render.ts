...
function renderAvatar(
  ...
  basePath?: string,
  userConfig?: { avatar?: string | null; onAvatarClick?: (e: Event) => void },
) {
  const normalized = normalizeRoleForGrouping(role);
  const assistantName = assistant?.name?.trim() || "Assistant";
  const assistantAvatar = assistant?.avatar?.trim() || "";
  const userAvatar = userConfig?.avatar;
  const onUserAvatarClick = userConfig?.onAvatarClick;

  // User avatar with click-to-edit support
  if (normalized === "user") {
    const hasCustomAvatar = userAvatar && isAvatarUrl(userAvatar);
    return html`
      <div
        class="chat-avatar ${normalized} ${hasCustomAvatar ? "chat-avatar--has-image" : ""}"
        @click=${onUserAvatarClick ?? nothing}
        title=${hasCustomAvatar ? "点击修改头像" : "点击设置头像"}
      >
        ${hasCustomAvatar
          ? html`<img src="${userAvatar}" alt="User avatar" class="chat-avatar__image" />`
          : html`
              <svg
                viewBox="0 0 24 24"
                fill="currentColor"
    ...
    normalized === "assistant"
      ...
      : normalized === "tool"
        ...
        : html`
          ...
          `;
  const className =
    normalized === "assistant" ? "assistant" : normalized === "tool" ? "tool" : "other";
